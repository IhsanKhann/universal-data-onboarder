// examples/offerberries-targets/party-seller.target.js
...
registerTarget({
  namespace: "businessops", entityKey: "party_seller", label: "Seller", icon: "Store",
  ...
  fields: [
    ...
    { key: "storeName", label: "Store Name", type: "string", required: false, aliases: ["store", "shop name"] },
  ...
  findExistingIdentities: async (tenantId, identityValues, conn = mongoose) => {
    const idType = await resolvePartyIdType(tenantId, "seller", conn);
    ...
    const existing = await getPartyModel(conn).find({ tenantId, partyType: "seller", externalId: { $in: c } }).distinct("externalId");
    return new Set(existing.map(String));
  },
  commitRow: async (tenantId, mappedFields, { conn = mongoose, session = null } = {}) => {
    const idType = await resolvePartyIdType(tenantId, "seller", conn);
    if (!mappedFields.externalId) throw commitErr('"External ID" is required for a seller row');
    const externalId = coerceFieldValue(idType, mappedFields.externalId, "External ID");
    if (!mappedFields.name) throw commitErr('"Name" is required for a seller row');
    const party = await createParty(tenantId, {
      partyType: "seller", externalId, name: mappedFields.name,
      ...
      sellerExt: { storeName: mappedFields.storeName ?? null, category: mappedFields.category ?? null },
    }, conn, session);
